...
**Free-trial form** — no server route. The trial form is a documented
exception to rule #9 ([[meta/decisions-log|ADR-0020]]): the client
submits directly to the Google Apps Script Web App URL held in
`NEXT_PUBLIC_TRIAL_ENDPOINT` (public by design — it is an unauthenticated
Apps Script deployment that xuralabs.com also submits to client-direct).
See [[frontend/components/sections|`trial-form.tsx`]] for the double-submit
(Image beacon + no-cors fetch) plumbing. Every other integration still
goes through `app/api/**` with server-only env vars.
