...
**Context.** [[frontend/architecture|AGENTS.md rule #9]] says external calls
run server-side in `app/api/**/route.ts` and secrets stay server-only. The
free-trial form was first built that way: `TrialForm` posted to
`/api/trial`, a Route Handler zod-validated and forwarded to a Google Apps
Script Web App using a server-only `TRIAL_ENDPOINT`. That is the safe
default and stays the default for anything with real secrets.

xuralabs.com's current marketing site does not do it that way. Its form
submits **directly from the browser** to the same Apps Script URL, twice for
durability: an `Image().src = SHEETS_URL + '?' + params` GET beacon *and* a
`fetch(SHEETS_URL, { method: "POST", mode: "no-cors", body: JSON.stringify(data) })`.
The URL is a Google Apps Script Web App bound to a Google Sheet; it is
public by design (deployed as *Execute as: Me · Who has access: Anyone*)
and is meant to receive unauthenticated writes.

**Decision.** Match xuralabs's plumbing exactly. `TrialForm` is now a client
component that submits directly to the Apps Script URL using the same
double-submit pattern. The old `/api/trial` route and server-only
`TRIAL_ENDPOINT` are removed. The URL is **hard-coded** as a `SHEETS_URL`
constant in the component itself — the project ships as a static-friendly
GitHub-hosted site with no env layer, and the URL is public by design, so
inlining it is simpler than a `NEXT_PUBLIC_*` env var. To redirect
submissions, deploy a new Apps Script and edit the one constant.

**Consequences.**
- A single project rule (AGENTS.md rule #9) is intentionally waived for this
  one form. Rationale: the endpoint is a *design-public* Apps Script URL
  bound to an unauthenticated write, not a secret. Exposing it in the client
  bundle costs nothing that going through our server would fix, and matching
  xuralabs's proven plumbing avoids surprises during the migration.
- Consumer email domains are blocked inline (matches xuralabs's block-list).
  Extra fields `source: "trial-page"` and `ts: <ISO>` are stamped at submit
  time so rows in the sheet stay attributable.
- The double-submit is belt-and-braces: even if one channel is blocked
  (ad-blockers strip beacons, network filters strip no-cors fetches), the
  other lands. Success is optimistic — no-cors responses are opaque, so we
  can't read status; we show the "You're in" state on submit and let the
  operations team verify in the sheet.
- Rule #9 still holds for every other external integration (CRM, contact
  forwarding, etc.) — those go through `app/api/**` with server-only env
  vars. This carve-out is scoped to the trial form only.

---

## ADR-0019 — AiOrb keyframe carveout (Xura's visual identity)

- **Status:** Accepted
- **Date:** 2026-08-09

**Context.** The AiOrb is Xura's product-wide visual identity — a squircle
"face" with cycling emotions, a floating brow, blinking eyes, a periodic 3D
spin, and a pulsing halo. It exists across every Xura surface (mobile shell,
Ask Xura, chat avatars). ADR-0002 forbids CSS keyframes for reveal / scroll /
staggered motion; ADR-0014 allows CSS transitions only for trivial discrete
state changes. Neither covers a self-contained *identity* whose entire visual
premise is continuous CSS animation.

**Decision.** The AiOrb keeps its CSS keyframes (blink, float, halo pulse,
scan, spin, face-shift, sheen-shift, body-turn, ring-out, active-flash).
Rationale: it is a discrete "graphical" component, not motion-in-a-layout;
porting it to springs would require re-implementing its 3D face parallax and
per-emotion morphs from scratch and would lose fidelity with the Xura app.

**Consequences.** Only the AiOrb and its child elements (`.m-orb__*`) are
covered by this carveout. Any new keyframes elsewhere still need an explicit
exception. If the orb's motion misbehaves (jank, over-work), work goes
through [[optimize-3d-scene]] first — the same performance discipline that
governs the [[decisions-log|ADR-0018]] canvas backdrop.

---

## ADR-0018 — Canvas rendering loops are exempt from the "all motion is spring-based" rule

- **Status:** Accepted
- **Date:** 2026-08-09

**Context.** The hero backdrop is a self-contained `<canvas>` scene driven by
`requestAnimationFrame` (a low-poly rotating wireframe ported from the Xura
app's post-login home). It has no DOM-visible transforms — every pixel is
drawn imperatively into the canvas context.

**Decision.** ADR-0002's spring-only rule targets React/DOM motion (transform,
opacity, layout, scroll-driven reveals). Continuous, self-contained canvas /
WebGL render loops that don't animate any DOM property are out of scope.
`HeroBackdrop` may keep its `rAF` loop; the [[optimize-3d-scene]] skill governs
performance work on such scenes.

**Consequences.** No spring wrapping of `<canvas>` scenes. Framework rules
still apply at the DOM boundary (mounts, opacity fades of the whole canvas,
show/hide). Any new canvas scene must honour `prefers-reduced-motion` and
should be considered by [[optimize-3d-scene]] before shipping.
...
## ADR-0021 — No cookie consent banner

- **Status:** Accepted
- **Date:** 2026-08-12

**Context.** The site sets no analytics, marketing, or fingerprinting cookies.
Only third-party service in play is a client-side POST to a Google Apps Script
Web App from the trial form. The former `components/common/Cookie/*` banner
stored nothing but its own consent flag in `localStorage`.

**Decision.** Remove the banner and the entire Cookie module. If a
tracking / analytics dependency is ever added, reintroduce a proper CMP
alongside it — not before.

**Consequences.** No visual consent surface. Adding any script that drops
cookies without a CMP would breach GDPR/ePrivacy; this ADR is the guard
rail against silently adding one.

---

## ADR-0022 — Ship this fork as a static export (GitHub Pages)

- **Status:** Accepted — fork-specific (only in `Xura-Web-v2-static`)
- **Date:** 2026-08-12

**Context.** The upstream `Xura-Web-v2` app is a full Next.js SSR project.
This fork exists to keep hosting on the existing `xura-marketing` GitHub
Pages repo + `xura.co` domain, avoiding introducing Vercel to the stack.

**Decision.** Turn on `output: "export"` in `next.config.ts`. Configure:
- `images.unoptimized: true` (Pages can't run the on-the-fly optimiser).
- `export const dynamic = "force-static"` on `app/icon.tsx`,
  `app/robots.ts`, `app/sitemap.ts`.
- Remove `app/api/contact/route.ts` (server routes cannot ship to Pages
  and the site does not call it — the trial form POSTs directly to a
  Google Apps Script from the client).
- `public/CNAME` (contains `xura.co`) and `public/.nojekyll` for Pages.

**Consequences.**
- Build writes the whole site to `out/`, ready to publish to a
  `gh-pages` branch.
- No server routes, no middleware, no ISR. If any of those is later
  needed, revert this ADR and move hosting to Vercel.
- `next/image` no longer transcodes / resizes. Pre-compress heavy
  images at source.
